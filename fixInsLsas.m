function [ argOut ] = fixInsLsas(dirMap,envs,objs,objData,ranges,aspects,chirp )
    ...
    for o = 1:length(objs)
        for r = 1:length(ranges)
            for a = 1:length(aspects)
                ...
                ac{6} = ['lsas.' num2str(ranges(r)) 'm.dat'];
                ...
                hs{6} = ['201 ' num2str(floor(2000*(sqrt(elv(e)^2+(ranges(r))^2))/c_w(e))), ' 20' ]; % Samp. rate, time offset, window len [kHz,ms,ms]
                ...
                hs{12} = ['lsas.' num2str(ranges(r)) 'm.dat'];
                hs{13} = 'file';
                ...
                td{10} = ['lsas.' num2str(ranges(r)) 'm.dat'];
